...
* WARRANTIES OF MERCHANTABILITY AND FITNSS FOR A PARTICULAR PURPOSE.
*/
...
static void     checkEnvironment(RobotPlayer* tank)
{
    ...
    //If I have a non-team flag then drop it
 //   if (tank->getFlag()->flagTeam == NoTeam)
 //   {
  ...
  //  }
   ...
   // if (flagd->flagTeam == RedTeam || flagd->flagTeam == GreenTeam  ) {
...
// have I captured a flag?
...
#endif
                //char buffer[128];
                //sprintf(buffer, "inside check Environment for base %d, team %d",base, team);
                //controlPanel->addMessage(buffer);
    ...
    const ShotPath* hit = NULL;
    float minTime = Infinity;
    tank->checkHit(tank, hit, minTime);
    int i;
    for (i = 0; i < curMaxPlayers; i++)
    {
    ...
    else
    {
        ...
        if (((tank->getFlag() == Flags::Steamroller) ||
            ((tank->getFlag() == Flags::Burrow) && tank->isAlive() &&
                !tank->isPhantomZoned()))
            && !tank->isPaused())
        {
            const float* pos = tank->getPosition();
            ...
            {
                const float radius = myRadius +
                    (BZDB.eval(StateDatabase::BZDB_SRRADIUSMULT) * tank->getRadius());
                const float distSquared =
                    hypotf(hypotf(myPos[0] - pos[0],
                        myPos[1] - pos[1]), (myPos[2] - pos[2]) * 2.0f);
                if (distSquared < radius)
                {
                    gotBlowedUp(tank, GotRunOver, tank->getId());
                    dead = true;
                }
            }
        }
